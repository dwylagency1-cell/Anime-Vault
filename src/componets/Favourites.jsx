...
function Favourites(){
    ...
    function RemoveFavourites(id){
        const NewFavourites = favourite.filter(function(items){
            return items !== id
    ...
    const favouriteWallpapers = wallpapers.filter(function (element) {
        return favourite.includes(element.id);
    });
    return <div className="min-h-screen w-full overflow-x-hidden pb-[64px] md:pb-0">
        ...
        <div className='pt-[80px] w-full ml-[220px] grid grid-cols-[300px_300px_300px] gap-x-[20px] gap-y-[30px] p-[20px] pl-[70px]'>
    {
        favouriteWallpapers.map(function(element){
            return (
                <div className="w-[300px] flex flex-col items-center">
                    ...
                    {/* Remove button OUTSIDE card */}
                    <button
                        onClick={function(){
                            RemoveFavourites(element.id);
                        }}
                        className="mt-3 bg-[#2A2A2A] text-white px-30 py-2 rounded-lg text-sm font-semibold transition-all duration-300 hover:bg-red-500 hover:shadow-[0_0_15px_rgba(239,68,68,0.35)] cursor-pointer"
                    >
                        Remove
                    </button>
